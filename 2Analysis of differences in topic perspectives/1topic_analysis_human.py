...
# from connection import MongoBase
import os
...
def topic(train, path, docs):
    ...
    topic_model.visualize_topics().write_html(path + 'v_topics' + '.html')  #这一行报错
...
def load_stopwords(stopwords_file):
    ...
    if os.path.exists(stopwords_file):
        with open(stopwords_file, 'r', encoding='utf-8') as file:
            for line in file:
                stopwords.add(line.strip().lower())  # 将停用词转换为小写
    return stopwords
